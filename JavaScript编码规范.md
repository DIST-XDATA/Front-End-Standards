...
#### 2.1.2 命名规则<h4 id="2.1.2"></h4>
    ...
    5. 私有（保护）成员，必须以下划线（_）开头，且必须采用camel命名法，例如：_element、_accountElement、_fSetValue()、_fGetValue()
...
### 2.2 特殊命名规范<h3 id="2.2"></h3>
    ...
    10. 重复变量建议使用i、j、k、l、m、n（依次类推）等名称的变量
    11. 补充用语必须使用补充词，例如：get/set、add/remove、create/destroy、start/stop、insert/delete、begin/end等
    12. 能缩写的名称尽量使用缩写
    13. 避免使用有歧义的布尔变量名称，例如：isNotError、isNotFound等
    14. 方法如果返回一个类，则应该在名称上说明返回了什么，如果是一个过程，则应该说明做了什么
---
## 3 详细命名规范<h2 id="3"></h2>
...
### 3.3 变量<h3 id="3.3"></h3>
    ...
    1.变量名应由26个大小写字母（a...z,A...Z），10个数字（0...9）和下划线（_）中的元素组成，避免使用国际化字符（如中文）
...
#### 3.4.3 语句标示<h4 id="3.4.3"></h4>
    ...
    1. return语句：一条有返回值的return语句不要使用括号来括住返回值，如果返回表达式，则表达式应与return关键字在一行
    2. if语句：应如以下格式
    if (condition) {
        statements;
    } else if (condition) {
    ...
    每次判断i是否达到临界值时都要获取一遍myArray的length属性，这样做影响效率，可以用以下的形式
    ...
    每个case相互对齐，每一组statements（除了default）都应以break、return或者throw结尾，不要让它顺次往下执行
    7. try语句：应如以下格式
    try {
        statements;
    } catch (variable) {
    ...
    8. with语句：可以用来引用某个特定对象中已有的属性，但不能用来给对象添加属性，要给对象创建新的属性，必须明确地引用该对象。在编写代码中尽量不要使用with语句
...
### 4.1 注释内容<h3 id="4.1"></h3>
    ...
    1. JS文件开头注释
    JavaScript文件在开头应包含类似以下注释说明，修改人、修改时间及修改描述这三项要实时更新，保证为最新
    /* 
    *@fileName:文件名 
    *@fileDesc:文件功能描述
    *@author:创建人 
    ...
    */ 
    2. 大功能区注释
    在大功能区（或难以理解的部分）代码的开头添加注释，对其功能、参数及返回值进行描述（具体详见JSDOC注释规范）
    /* 
    *@desc:功能描述 
    *@param:参数描述 
    *@return:返回值 
...
## 5 其他注意事项<h2 id="5"></h2>
    ...
    3. 在方法顶端定义所有变量，在大部分情况下var语句应该为方法体内的第一个语句
    4. 尽量少使用全局变量，隐式的全局变量应该从来不使用，同时也应尽量避免全局函数的使用
    5. 代码结构应该提供最好的可读性
    6. 不要在命名中使用里使用$，把它留给JQuery，也不要使用反斜杠（/）
    7. 在每个简单语句末尾添加一个;(分号)
